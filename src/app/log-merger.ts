...
export class LogMerger {
    public merge(logs: Log[]): LogLine[] {
        ...
        while (true){
            ...
            {
                var index = indices[i];
                if (index < logs[i].lines.length
                    && (nextLog == -1
                        || logs[i].lines[index].timestamp < logs[nextLog].lines[indices[nextLog]].timestamp))
                {
                    nextLog = i;
                }
            }
            ...
            if (prevLine == null || prevLine.text != nextLine.text || prevLine.Log != nextLine.Log)
            {
                lines.push(nextLine);
            }
